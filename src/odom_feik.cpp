    ...
    void vision_callback(const aruco_interfaces::msg::ArucoMarkers &markers) {
      ...
      for(i = 0; i < markers.marker_ids.size(); i++) {
        ...
        if(world_poses.find(aruco_id) != world_poses.end()) {
          valid = true;
          z_hat << sqrt(pow(world_poses[aruco_id][0] - x_hat(0), 2) + pow(world_poses[aruco_id][1] - x_hat(1), 2)),
                   atan2(world_poses[aruco_id][1] - x_hat(1), world_poses[aruco_id][0] - x_hat(0)) - x_hat(2);
          ...
          H << (x_hat(0) - world_poses[aruco_id][0]) / z_hat(0), (x_hat(1) - world_poses[aruco_id][1]) / z_hat(0), 0,
               (world_poses[aruco_id][1] - x_hat(1)) / pow(z_hat(0), 2), (x_hat(0) - world_poses[aruco_id][0]) / pow(z_hat(0), 2), -1;
          
          y_hat << sqrt(pow(aruco_pose.position.x, 2) + pow(aruco_pose.position.z, 2)) - z_hat(0),
                   atan2(-aruco_pose.position.x, aruco_pose.position.z) - z_hat(1);
